...
schema, the enumerated edge set, and the closed enums. (28 symbols)
...
*instances* without ever re-touching the schema. (10 symbols)
...
- **_main.py** — gate — a reusable, programmatic gate predicate. (5 symbols)

### `agency/capabilities/intent/` (2 files)
- **__init__.py** — intent — Spec-091 critical-thinking capability. (2 symbols)
- **_main.py** — intent — critical-thinking methods that reason about the serving intent (Spec 026/091). (15 symbols)
...
Read, Update, Amend and Retract without per-capability code. (30 symbols)
...
`_main` module docstring — that's the single source the engine derives
from (CapabilityBase.as_capability → SkillDoc.from_module). (3 symbols)
- **_main.py** — novel — minimum-viable-novel Slice 1 (Spec 101 master First-Principles Minimum). (63 symbols)
...
all derive from one source. (22 symbols)
...
Dropped (implementation / structural — not observable behaviour):
  ...
  - test_document_scope_guards.test_paths_scan_rejects_non_intent_root — tests
    ...
    (covered in analyze acceptance). (135 symbols)
...
- **test_plugin_authoring.py** — Acceptance — plugin authoring behaviour. (66 symbols)
...
- **test_render_substrate.py** — Acceptance — render substrate and response envelope (Spec 023 / 146 / 154). (66 symbols)
...
Dropped as implementation/structural (not observable behaviour):
  test_skill_walk_slices.py — render_phase / render_verb output format tests
    ...
    internal disclosure-helper details. (37 symbols)
- **test_skill_walk.py** — Acceptance — skill walk behaviour. (31 symbols)
...
Dropped as implementation/structural (not observable behaviour):
  test_skills_api_binding.py — verifies the Anthropic Python SDK signature;
    depends on [publish] extra that CI doesn't install. (31 symbols)
