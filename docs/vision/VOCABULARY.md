...
| **Engine** | The substrate (NOT a concept) — one FastMCP server + one graph. **Code-mode IS the contract**: the public surface is exactly `search · get_schema · execute`. Capabilities self-register by reflection. The host. |
