...
| **Harness ladder (L1/L2/L3)** | Execution tiers — in-process, subprocess, sidecar daemon + CLI — that all speak the four-verb contract. |
...
| **Artifact driver** | A backend mapping `Artefact` nodes to external storage (`fs`, `repo`, `s3`, `http`, `drive`). |
