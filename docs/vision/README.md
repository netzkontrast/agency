...
Vision canon, the Plan — and the first running code** ([`seed/`](../../seed/README.md)).
...
5. **[EXAMPLE.md](EXAMPLE.md)** — a worked walkthrough ("fix the failing auth
   test, via jules"), **now executable** in [`seed/`](../../seed/README.md).
...
The canon DOCUMENTS the entire four-concept model. The first RUNNING code is
[`seed/`](../../seed/README.md) — a proof-of-concept (not the shipped engine)
that proves the moat (cross-concern provenance in one traversal), code-mode
tool-chaining, and gate/elicitation human-in-the-flow. Everything else is
**"specced — not built."**
