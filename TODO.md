...
| 094 | music-lifecycle-cluster | **Partial** | 14 verbs covering 22 bitwize lifecycle tools via StateDriver; carries the migration (examples/music.py → agency/capabilities/music/) + doctrine exception (CLAUDE.md + CAPABILITY-CLUSTERS.md) | **Slice 1 SHIPPED** on `claude/busy-bohr-wb18pg`: migration only (folder layout, `_main.py` + `ontology.py` + `drivers.py` + cluster stubs, 5 lifecycle templates ported verbatim from bitwize, deprecation shims, conftest+pyproject markers for 7 music subclusters, smoke tests green). **Still:** 14 NEW lifecycle verbs (`create_album`, `find_album`, `promote_idea`, `list_ideas`, `create_track`, `list_tracks`, `set_track_status`, `rename_album`, `rename_track`, `album_progress`, `resume_session`), StateDriver method delta + fake, 50 reference docs + all 388 genres vendored (mechanical — subagent), `tests/test_music_lifecycle.py` expanded to ~12 cases, full Done-When per spec |
