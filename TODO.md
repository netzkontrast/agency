...
| **Shipped — audit-trail (`intent:558f1bf5` / `intent:d8090aef`)** | 3 | **073 ✅ → `shell` capability** (run/filter/templates) · **018 (Wins 1+3) ✅** (`develop.skill_walk` atomic walker + implicit `intent_id` via `AGENCY_INTENT`; Win 2 = cancelled-069, skipped) · **075 ✅** (`shell.define` + `shell.templates(query)` + run resolves graph-first; common-bash seeds — definable registry per CLAUDE.md #8) |
| **Drafted — next-wave roadmap (user-sketched 2026-06-06)** | 4 | 076 unified-event-hook (research-first) · 077 bdd-gherkin-tests (research-first) · 078 static-walkable-skills (research-first; needs clarification) · **079 click-cli-verb-mirror** (Click CLI exposing every verb as a command; code-mode stays canonical) |
