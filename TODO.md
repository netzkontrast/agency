...
| 063 | venv-fallback-install-path | **Shipped** | 3-step install fallback chain (pipx → pip --user → per-project venv at `${CLAUDE_PROJECT_DIR}/.agency/.venv`); `bin/agency-mcp` + `bin/agency` shims grew a venv-first check before PATH iteration; hook runs `agency.install --scaffold-db` after any successful install path so target repo gets `.agency/` initialized; covers sandboxed Web envs without pipx | 4 new tests green (2 shim resolution + 2 hook fallback); 17/17 install slice green; README "Fallback chain" block expanded |
