...
- 166  analyze-deps-extras-expand   # Partial — wrapper registry now DERIVES from the modules: external_tools() reads EXTERNAL_TOOL off _ruff/_bandit/_radon (retires the hand-listed analyze_extras AGENCY-DRIFT tuple), derive_wrapper_shapes() -> typed WrapperShape, agency_doctor.wrapper_coverage; TDD (5). Still: the 3 NEW linter wrappers (mypy/pylint/semgrep) + semgrep cross_cap_import gate + ANALYZER_PARTIAL (the linter-expansion headline)
...
- 172  analyzer-linter-expansion   # Shipped (done) — derive_axis_registry() composes live analyzer AXIS_PREFIXES into the typed AxisRegistry (longest-prefix-first, order-independent), detect_collisions + AXIS_PREFIX_COLLISION/AXIS_PREFIX_MALFORMED guards, agency_doctor.axis_registry_coverage (20 entries, 0 collisions); TDD (7 tests), check-drift clean
...
- 387  loop-activation
- 388  jinja-template-engine   # Shipped (done) — core shipped (owner directive): jinja2 core dep, ctx.render → Jinja Environment (StrictUndefined, autoescape off, FunctionLoader for {% include %}), quality-report/iron-law-finding/improvement-plan/research-report ported ({% if %}/{% for %}/{# #}), interim Spec-384 strippers DELETED + render_quality_report shrunk. TDD (9) + full suite 1742 green. MERGED via PR #323; done-cascade run (owner AdR.approve → Plan/done/, CAPABILITIES-06 approved, architecture.md rebuilt to 21 decisions). Follow-up complete (owner "Implementierung Both"): remaining 8 $var templates ported to {{ }}, dogfood.render moved onto ctx.render (regex comment-strip deleted), standing no-$var-left test. NOTE: pre-existing spec_id collision with 388-spec-done-cascade.
- 389  derived-fence-reference-docs   # Shipped (done) — scripts/derive_docs.py gains 3 code-introspection fence kinds (substrate-tools, driver-boundaries, capability-verbs:<cap>) deriving from live code, apply_doc_fences/doc_has_derived_drift/derive_docs_pass + --write-docs/--check-docs CLI (wired into check-drift). check-doc-drift logic extracted to importable scripts/_doc_drift.py + triages STALE-AUTO (derived, auto-fix) vs STALE (prose, hand-review). overview.md substrate-tools list migrated to a derived fence (proof + standing guard). TDD (19), check-drift clean. MERGED via PR #324; done-cascade run (owner AdR.approve → Plan/done/, WORKFLOW-04 approved, architecture.md rebuilt). Follow-up complete (owner "Implementierung Both"): drivers.md boundary roster fenced (driver-boundaries), overview.md de-numbered, live-discovered standing guard over every fenced doc.
