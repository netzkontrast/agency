...
| **Spec-panel approved — design ready (goal program)** | 3 | **081 walkable-usage-skills** (derived `<cap>-usage` phase-graphs teaching the MCP surface — NEXT, TDD) · **082 count-tokens-budget** (replace tiktoken with `count_tokens` in the budget gate) · **083 publish-skill-api** (`plugin.publish_skill` → Anthropic Skills API) |
