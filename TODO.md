...
| 059 | toolresult-convenience-layer | Not started | Codes namespace + `.success`/`.failure` ctors + `Registry.invoke` stamps `error.trace_id` via `dataclasses.replace` + `next_cursor` opt-in + "when to use ToolResult vs plain dict" doctrine | Drafted 2026-06-03 as carry-over from Spec 001 after its Q-2 territory was superseded by Spec 019. Depends on 001 + 019. |
