...
| `lifecycle_trail` | act | LIFECYCLE WATCH — the Spec 341 `watch` frame: the ordered Spec 344 `lifecycle_transition` Event trail recorded `OBSERVED_DURING` a lifecycle (what `move` emits), so an observer reads the durable history instead of polling `state`. | [details](references/lifecycle_trail.md) |
