...
def test_welcome_token_budget_still_under_2kb_in_progress():
    ...
    # Owner directive (2026-06-13): constant overhead → 2000 (took the higher of
    # the two parallel-session bumps — 1300 for Spec 282 G's `sandbox_constraints`
    # vs 2000 for the wider Spec 282/284/285 fixed-envelope growth). Fixed
    # overhead, not per-cap gist bloat; per-cap coefficient (150) unchanged, so it
    # still guards bloat (rule 8: tunable named budget with rationale).
