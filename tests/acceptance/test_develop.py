...
# ── develop.port_plugin — external plugin porter (Spec 293) ───────────────────

@given(parsers.parse('an external plugin directory with command files "{a}", "{b}", and "{c}"'),
       target_fixture="plugin_dir")
def _plugin_dir(tmp_path, a, b, c):
    d = tmp_path / "extplugin"
    d.mkdir()
    for name in (a, b, c):
...
@when("I call develop.port_plugin on that directory", target_fixture="port_result")
def _port(engine, confirmed_intent, plugin_dir):
    r, _ = engine.registry.invoke(engine.memory, confirmed_intent, "develop",
                                  "port_plugin", agent_id="agent:test",
                                  source=str(plugin_dir))
    return r


@then("the port result ingests three Documents")
def _port_three(port_result):
    assert port_result["ported_count"] == 3, port_result
    assert all(p["document_id"].startswith("document:") for p in port_result["ported"])


@then(parsers.parse('the port gap-map covers "{covered}" and flags "{gap}" as a gap'))
def _port_gapmap(port_result, covered, gap):
    assert any(c["name"] == covered for c in port_result["covered"]), port_result
    assert gap in port_result["gaps"], port_result


@then("the external files are left without an anchor")
def _no_anchor(plugin_dir):
    for f in plugin_dir.glob("*.md"):
        assert not f.read_text(encoding="utf-8").startswith("<!-- agency-node:")
