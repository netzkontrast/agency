...
# AGENCY-DRIFT: shell-templates — named query recipes (write-less). The agent calls
# run(template="…") instead of composing the command + filter each time.
_TEMPLATES = {
...
class ShellCapability(CapabilityBase):
    ...
    def templates(self) -> dict:
        """List the named query templates (write-less common commands).

        Inputs: none.
        Returns: ``{result: [{name, command, filter, doc}, …]}``.
        chain_next: ``shell.run(template=<name>)``.
        """
        return {"result": [{"name": n, **t} for n, t in sorted(_TEMPLATES.items())]}
