...
# Spec 374 — SARIF emit + CI step + the quality gate + report rendering
...
property test, Spec 375 — not a parser test.)
...
- **`rules`** ← the live risk-code registry (Spec 354 `decay-risks.json` + any
  ...
  = diagnostic question, `helpUri` → the source-coverage doc (375), `properties`
  carry the book sources. The rule set is **derived from the live registry**, not
  a literal list — so it never drifts (rule 8; tested in 375).
...
`develop.review` / `analyze.review` (372) end by recording a gate:
...
Spec 373), documented tunable budgets (rule 8). A failed gate flips the lifecycle
...
`analyze.review` — the headless twin (372 §3a)** — never `develop.review`, because
...
- **Trend in ephemeral CI (Hightower fix).** The graph DB is otherwise fresh each
  run, so the 373 `QualityRun` trend would always read "first run". The job
...
Spec 376** (the prose port); this slice owns the *render path* that projects it
from the graph. The template carries these as `<!-- AGENT: -->` blocks:

- Header: `Mode · Scope · Health Score [· Trend]` (trend from 373's `QualityRun`).
...
- **Summary** — 2–3 sentences; under `legacy-friendly`, lead with the three
  highest-leverage fixes (Spec 373 §1).
- A collapsed **Suppressed** section (Spec 373 §4) when suppressions matched.
...
- No score math (373) / no finding production (354/372) — it renders + gates them.
...
**DRAFTED 2026-06-20** — the CI/output slice of the Spec 371 program. No code yet.
Reuses `gate` (codegraph: gate/_main.py:24/52), `document.render` (Spec 292), the
CLI mirror (Spec 079), and `test.yml`. Depends on 354 (findings) + 373 (score).
...
twin (372 §3a), never the interactive `develop.review` (Cockburn). Added:
keyless decidable-only degradation when no LLM secret (Hightower); `.agency` graph
cache keyed by base branch so the 373 trend survives ephemeral CI (Hightower);
...
reader verb). Next (after 354/373): `_sarif.py` + `analyze.sarif` (+ `--max-results`)
