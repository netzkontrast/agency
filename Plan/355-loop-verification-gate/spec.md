...
owner: "@agency"
vision_goals: [2, 5]
depends_on: ["192", "285", "328", "353", "356"]
parent_spec: "353"
domain: loop / gate
wave: looper-port
---

# Spec 355 — Loop verification as typed gates

> Child of Spec 353. Ports looper's **verification stage** +
...
> capability with a typed `GateResult`. 355 expresses looper's three-way criterion
> taxonomy on it.
...
fulfilment gate). 355 maps each looper criterion type to a gate evaluation path —
...
def add_criterion(self, ctx, loop_id: str, kind: str, *, check: list[str] | None = None,
    ...
    kind ∈ {programmatic, judge, human}:
      ...
      judge        → rubric (scored by a council member, 356)
      human        → prompt (shown at an elicit checkpoint)
    Returns {criterion_id, kind}. chain_next: loop.add_member (356) for judge
...
| `judge` | delegates to **356** `loop.convene` — a council member scores the rubric, returns a structured verdict | the council member (a persona+driver) |
...
path produces the **same typed verdict** the loop machine (357) consumes, and
...
> The parser is owned by 356; 355 consumes its typed result.
...
(looper Emit Checklist) is surfaced as a warning the wizard (358) can hard-gate.
...
Scenario: a judge criterion delegates to the council and returns a typed verdict
  Given a judge criterion and a council member (356)
...
- [ ] judge path delegates to 356 and tolerates unparseable output (degrade + warn).
...
**Verdict:** Not started — drafted under the 353 wave. The three-way criterion
taxonomy expressed on the `gate` capability (328): programmatic via `gate.check`,
judge via the 356 council, human via `elicit` (285). Verdict degradation parity
with looper. Depends on 356 for the judge path; consumed by the machine (357) and
the wizard (358).
