...
owner: "@agency"
vision_goals: [2, 3, 6]
depends_on: ["371", "354", "081", "080", "018", "046"]
domain: develop
wave: brooks-port
parent_spec: "371"
---

# Spec 372 — Six quality modes as walkable skills + the `develop.review` seam

> Part of the Spec 371 brooks-lint port. This slice is the **"integrate linting
...
default), authored to walk the **hybrid shape** from Spec 371 §3. The SkillDoc
...
Naming uses the `quality-*` prefix (the agency lens name from Spec 371 §1), not
...
Every mode skill walks the Spec 371 §3 phases, with mode-specific bodies:
...
4. **score+report** — Health Score (373) + render the Iron Law report (374).
5. **remedy/triage** (sweep + `--fix` only) — §3 below; the triage step calls
   `intent.triage` (Spec 373 §4 — triage is an intent judgment), interactive only.
...
from a `develop/templates/quality-{mode}.md` template (Spec 376), not inlined in
...
via `develop.reference(...)` (Spec 376).
...
CI uses (Spec 374). This dissolves OQ3.
...
> that `(risk_code, span)`. So the report never double-counts and the score (373)
...
> This is a documented v1 limitation with an evolution path — **Spec 378**
...
> language coverage in its Scope line — computed by 378's `analyze.coverage()`
...
- The `remedy-guide.md` discipline (brooks `_shared/remedy-guide.md`) is read on
  demand before writing fixes (vendored as `develop.reference("remedy")`, Spec 376).
...
- No score math (373) — phase 4 *calls* it.
- No SARIF / CI (374) — but the `analyze.review` headless core (§3a) is what 374
  calls; this slice defines it, 374 wires it.
...
**DRAFTED 2026-06-20** — the develop-integration slice of the Spec 371 program.
...
**Remaining in 372:** the implement/tdd cross-link (optional review phase before
pre-commit gate); `develop.reference("brooks")`/`("decay-risks")` entries; CLAUDE.md
verb-routing row for `develop.review`. Next sibling: **373** (Health Score + config +
`quality:` config block + `QualityRun` graph nodes for trend). The `analyze.review`
headless core defined here is what **374** (SARIF + CI gate) calls.
