...
owner: "@agency"
vision_goals: [4, 2]
depends_on: ["379", "360", "380", "042"]
domain: analyze
wave: brooks-port-followup
parent_spec: "379"
---

# Spec 386 — Multi-language decidable scanners (the post-v1 evolution path)

> Followup to the Spec 379 brooks-lint port. v1 ships **decidable tagging =
> Python, judgment = any language** (Spec 380 §3c, owner-confirmed 2026-06-20).
...
> alone. Explicitly **out of the initial build order** (360→385) — it lands when a
> non-Python codebase actually needs the decidable lift, not before (YAGNI).

## Why

The hybrid engine (Spec 379 §3) has two halves with different language coupling:
...
documented limitation) — but it caps the decidable→judgment bridge (360 §3) to
...
`decidable` entries in `decay-risks.json` (360 §2) — e.g. R1's `decidable` array
grows `["long_function", "ts:long_function", "go:long_function", …]`. `_decay.py`
(360 §3) tags them identically; the merge contract (380 §3b) dedups against
judgment the same way. One risk registry, N language scanners feeding it.

### 4. The language matrix becomes live, not static

Spec 380 §3c states the matrix in prose; this spec makes it **computed** — the
...
go: []}`). So "decidable coverage = none for this language" (380 acceptance) is a
...
- **No judgment changes** — judgment is already any-language; this is only the
  decidable half.
- **No `Finding`/risk-registry changes** (360 owns them) — scanners feed the
...
**DRAFTED 2026-06-20** — the post-v1 evolution slice of the Spec 379 program,
...
in the v1 build order** (360→385) — it lands when a non-Python codebase needs the
...
existing `Finding` shape + `decay-risks.json` `decidable` arrays (360) + the merge
contract (380 §3b); makes the 380 §3c language matrix a computed `analyze.coverage()`
query. Depends on 360 (shape/registry) + 380 (matrix concept). Next (when
