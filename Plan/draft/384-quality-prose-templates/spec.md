...
owner: "@agency"
vision_goals: [4, 2, 6]
depends_on: ["379", "360", "380", "382", "060", "292"]
domain: analyze
wave: brooks-port
parent_spec: "379"
---

# Spec 384 — Brooks prose → agency templates + references (the develop-lint output surface)

> Part of the Spec 379 brooks-lint port. This slice carries the **prose** —
...
decay and HOW to shape the report. Specs 360–383 port the structured half
(findings, score, SARIF, config, evals); without the prose they are a skeleton
with no diagnostic voice. "Use everything from the Brooks-Lint system" (Spec 379)
...
|---|---|---|
| shapes **output** (the report a human reads) | a render **template** (`<cap>/templates/`) | `ctx.render` → `document.render` (382) |
| shapes **judgment during the walk** (how to diagnose, what NOT to flag) | an on-demand **reference** | `develop.reference("<topic>")` |
| is **fact/registry** (risk defs, book matrix, score weights) | **data** (`<cap>/data/*.json`) | 360 / 381 / 383 |
...
template, rendered graph→file by `document.render` (382 §4). It carries the
...
<!-- AGENT: render the Config: line here ONLY when a config was applied
     (preset, N disabled, M ignored) — Spec 381 §2. Omit otherwise. -->
...
<!-- AGENT: collapsed "Suppressed" section when suppressions matched (381 §4);
     not counted in the score. -->
$suppressed_block

## Summary
<!-- AGENT: 2–3 sentences. Under legacy-friendly, LEAD with the three
     highest-leverage fixes (381 §1). -->
$summary
...
<!-- AGENT: $fix_tier_label is the [quick-fix]/[guided]/[manual] tag ONLY under
     ...
     Consequence or Remedy is REJECTED upstream (the Iron Law gate, 380 §2) —
     this template never renders a partial finding. -->
```

This makes the Iron Law a **rendered invariant**: the template has four required
slots; the judgment gate (380) guarantees they are filled before render.
...
The mode skill (380 §2) renders its template at the matching phase, so the engine
...
- `develop/templates/quality-remedy.md` ← `remedy-guide.md`: the
  ...
  in the remedy phase (380 §4, `--fix`). Carries brooks' "What NOT to do"
...
|---|---|---|
| `"decay-risks"` | `decay-risks.md` + `test-decay-risks.md` | before the judgment pass (380 §2 phase 3) |
| `"source-coverage"` | `source-coverage.md` | before writing findings (the cite-discipline, 383) |
| `"remedy"` | `remedy-guide.md` (the heavy how-to) | before `--fix` (paired with §4's template) |
| `"custom-risks"` | `custom-risks-guide.md` | when config declares `custom_risks` (381 §2) |
...
> **facts** → JSON data (360/381/383). The reference prose POINTS AT the data
...
- **No finding/score/SARIF logic** (360/381/382) — it renders THEIR output and
  guides THEIR judgment; it ports prose, not engine.
- **No risk definitions in prose** — those are data (360); references point at the
...
- **No prose-parsing** (`report-parse.mjs` stays dropped, 382) — templates render
...
**DRAFTED 2026-06-20** — the prose/output slice of the Spec 379 program, opened by
...
templates (render output), references (guide judgment), and data (360/381/383).
Depends on 360 (Iron Law fields the template renders), 380 (mode skills that bind
the walk templates), 382 (`document.render` path). Lands alongside 382/383. Next
(after 360/380): author `quality-report.md` + `iron-law-finding.md` +
