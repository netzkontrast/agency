...
owner: "@agency"
vision_goals: [4, 2]
depends_on: ["371", "354", "373", "292"]
domain: analyze
wave: brooks-port
parent_spec: "371"
---

# Spec 377 — Migration: brooks-lint plugin → native agency quality surface

> Part of the Spec 371 brooks-lint port. This slice is the **transition path** for
...
Spec 371 "replaces brooks-lint itself" — but a project that *already runs*
brooks-lint has state on disk: a `.brooks-lint.yaml` config and a
`.brooks-lint-history.json` trend. The port specs (373) say these "disappear" —
...
- Maps each key onto the unified `quality:` block (373 §2), preserving semantics
...
- **Compat window:** 373 already reads `.brooks-lint.yaml` for one release
  (373 OQ1), agency config winning on conflict. This verb makes the migration
  *explicit + durable*; the auto-read is the *safety net* during the window. After
  the window, the auto-read is removed (a 373 follow-up) and the file is inert.
...
- Mints one `QualityRun{mode, scope, score, critical, warning, suggestion,
  recorded_at, status: complete}` per record (373 §3), back-dating `recorded_at`
...
1. **Native lands** (354–376 shipped) — `develop.review` live, both surfaces
   coexist (the native uses `quality-*` names; brooks uses `/brooks-*` — Spec 372
...
4. The brooks-lint **repo stays** as the cited upstream the data is vendored from
   (Spec 371 — like ponytail for `frugal`); deinstalling the *plugin* ≠ deleting
   the *reference*.

### 4. Vendored-data versioning (cross-ref — owned by 354/375)

The `_source: brooks-lint@<rev>` provenance keys on `decay-risks.json` (354) and
`source-coverage.json` (375), checked by `check-drift`, are the *forward*
...
- No new config schema (373 owns `quality:`) — it *writes* it from the legacy file.
- No new `QualityRun`/`Suppression` shape (373) — it *imports into* them.
- No destructive deletion of the legacy files (keep-both; the user removes them).
- No permanent dual-read shim — the 373 auto-read is a bounded compat window; this
  verb is the durable migration that lets the window close.
- No re-vendoring of the decay-risk data (354/375 own the data + its `_source`).
...
Scenario: suppress entries become Suppression nodes via intent.triage
  ...
  And the next review downgrades that finding (373 §4)
...
**DRAFTED 2026-06-20** — the migration/transition slice of the Spec 371 program,
opened by the spec-panel critique (Newman — the completeness gap the critical-
thinking pass and the panel both flagged). No code yet. Reuses 373 (`quality:`
config + `QualityRun`/`Suppression`), `intent.triage` (373 §4), keep-both (292).
...
the vendored-data `_source` read. Lands alongside 375/376 (late — needs 373's
config + nodes live). Next (after 373): the two importers + the first-review
