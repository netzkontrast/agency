...
owner: "@agency"
vision_goals: [4, 2]
depends_on: ["371", "354", "372", "042"]
domain: analyze
wave: brooks-port-followup
parent_spec: "371"
---

# Spec 378 — Multi-language decidable scanners (the post-v1 evolution path)

> Followup to the Spec 371 brooks-lint port. v1 ships **decidable tagging =
> Python, judgment = any language** (Spec 372 §3c, owner-confirmed 2026-06-20).
...
> alone. Explicitly **out of the initial build order** (354→377) — it lands when a
> non-Python codebase actually needs the decidable lift, not before (YAGNI).

## Why

The hybrid engine (Spec 371 §3) has two halves with different language coupling:
...
(354 §3) tags them identically; the merge contract (372 §3b) dedups against
judgment the same way. One risk registry, N language scanners feeding it.

### 4. The language matrix becomes live, not static

Spec 372 §3c states the matrix in prose; this spec makes it **computed** — the
...
go: []}`). So "decidable coverage = none for this language" (372 acceptance) is a
...
**DRAFTED 2026-06-20** — the post-v1 evolution slice of the Spec 371 program,
...
in the v1 build order** (354→377) — it lands when a non-Python codebase needs the
...
contract (372 §3b); makes the 372 §3c language matrix a computed `analyze.coverage()`
query. Depends on 354 (shape/registry) + 372 (matrix concept). Next (when
