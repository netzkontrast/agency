...
owner: "@agency"
vision_goals: [2, 4]
depends_on: ["379", "360", "334", "336", "290", "091"]
domain: analyze
wave: brooks-port
parent_spec: "379"
---

# Spec 381 — Health Score + config + history + triage
...
(Spec 379: "use everything") — but agency already has better homes for two of
...
`tier` is the **derived** mapping from `FindingSeverity` (Spec 360 §1) — the score
...
concern owned by 382, parameterized here. **"Leverage" is a defined, computed
...
| `custom_risks: {C1: {...}}` | merge `Cx` definitions into the decay-risk registry (Spec 360 §4) |
...
- **Trend in ephemeral CI (Hightower fix).** A fresh CI container has an empty
  ...
  382 §3 **caches `.agency/` keyed by the base branch** so prior `QualityRun` nodes
...
- **Incomplete runs are recorded but excluded from trend (Nygard fix).** A walk
  ...
  the latest point.
- The report's Trend line (382) is derived from the prior same-mode **complete**
  `QualityRun`.
- **No `.brooks-lint-history.json`** — it disappears (port-forward, like
  `report-parse` in 382). Its one-time import is owned by the migration slice
  (Spec 385).
...
- Interactive triage (the one-finding-at-a-time prompt) is the **Post-Report
  Triage** phase of the mode skill (380), guarded to interactive sessions only
...
- No report rendering / SARIF (382) — it computes the score + the suppressed set;
  382 renders them.
- No new findings — it scores/filters the findings 360/380 produce.
...
**DRAFTED 2026-06-20** — the scoring/config/triage slice of the Spec 379 program.
No code yet. Reuses Spec 334 (unified config), Spec 290 (`manage.timeline` for
trend), Spec 336/292 (keep-both, no sidecar files). Depends on 360 (the `tier`
...
382's base-branch `.agency` cache, with "first run" as the honest miss-fallback
(Hightower); `.brooks-lint-history.json` one-time import moves to the new migration
slice (Spec 385). Next (after 360): `_score.py` + `score-presets.json` + the
...
`tier` reads `Finding.tier` (Spec 360 §1 — one severity source); unknown preset
