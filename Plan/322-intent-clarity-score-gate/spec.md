...
### Done — Slice 3 (guided-discovery discipline, Spec 323 wiring)

- **`GUIDED_DISCOVERY_SKILL`** added to `agency/capabilities/discover/ontology.py`:
  - 7-phase authored discipline dict with `name="guided-discovery"`,
    `kind="discipline"`, `applies_when` pattern trigger.
  - Phase 7 ("decide") carries `gate="computed"` + `gate_verb="clarity_gate"` —
    making the clarity gate a structural property of the skill walk
    (unbypassable by the walker, not a reviewer's hope).
  - Registered in `discover_ontology = OntologyExtension(... skills={"guided-discovery": ...})`.
  - Per Spec 081: authored `ontology.skills` suppresses the derived `discover-usage`
    walk (capability.py condition `not getattr(ontology, "skills", None)` → False).
- **Regression fix** in `tests/acceptance/test_discover.py` + `discover.feature`:
  updated `_usage_walk` to assert `"guided-discovery"` (not `"discover-usage"`)
  since the authored discipline now replaces the derived default — correct per 081.
- **6 new acceptance scenarios** in
  `tests/acceptance/features/guided_discovery_skill.feature` +
  `tests/acceptance/test_guided_discovery_skill.py`:
  1. `parse_skill` returns success (no structural defects).
  2. kind is `"discipline"`.
  3. 7 phases, all with non-empty `produces`.
  4. Phase indices are contiguous 1..7.
  5. Final phase name/gate/gate_verb (`decide` / `computed` / `clarity_gate`).
  6. `"guided-discovery"` registered in engine ontology skills.
- `skills/discover/SKILL.md` auto-regenerated by `python -m agency.install`
  (now shows the discipline with all 7 phases, not a derived usage skill).
- All 13 Spec 322 acceptance scenarios green (3 Slice-1 + 4 Slice-2 + 6 Slice-3);
  31/31 discover scenarios green; drift clean; doc-drift clean.
