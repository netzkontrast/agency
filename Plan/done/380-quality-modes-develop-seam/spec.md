...
owner: "@agency"
vision_goals: [2, 3, 6]
depends_on: ["379", "360", "081", "080", "018", "046"]
domain: develop
wave: brooks-port
parent_spec: "379"
---

# Spec 380 — Six quality modes as walkable skills + the `develop.review` seam

> Part of the Spec 379 brooks-lint port. This slice is the **"integrate linting
...
default), authored to walk the **hybrid shape** from Spec 379 §3. The SkillDoc
...
Naming uses the `quality-*` prefix (the agency lens name from Spec 379 §1), not
...
Every mode skill walks the Spec 379 §3 phases, with mode-specific bodies:
...
2. **decidable** — call `analyze.quality/security/performance/architecture`; tag
   findings with risk codes (Spec 360 `_decay.py`).
3. **judgment** — read `decay-risks.json` (360) for the reasoning-heavy risks;
   ...
   the structured `Finding` fields (360 makes them structured precisely so this is
...
4. **score+report** — Health Score (381) + render the Iron Law report (382).
5. **remedy/triage** (sweep + `--fix` only) — §3 below; the triage step calls
   `intent.triage` (Spec 381 §4 — triage is an intent judgment), interactive only.
...
from a `develop/templates/quality-{mode}.md` template (Spec 384), not inlined in
...
via `develop.reference(...)` (Spec 384).
...
CI uses (Spec 382). This dissolves OQ3.
...
> that `(risk_code, span)`. So the report never double-counts and the score (381)
...
> This is a documented v1 limitation with an evolution path — **Spec 386**
...
> language coverage in its Scope line — computed by 386's `analyze.coverage()`
...
- Each `Finding`'s `remedy` (Spec 360) is classified safe vs risky (safe =
...
- The `remedy-guide.md` discipline (brooks `_shared/remedy-guide.md`) is read on
  demand before writing fixes (vendored as `develop.reference("remedy")`, Spec 384).
...
- No score math (381) — phase 4 *calls* it.
- No SARIF / CI (382) — but the `analyze.review` headless core (§3a) is what 382
  calls; this slice defines it, 382 wires it.
- No new decidable detectors — judgment reads 360's data; decidable is 360's
  bridge.
...
**DRAFTED 2026-06-20** — the develop-integration slice of the Spec 379 program.
...
`SkillRun` provenance path (Spec 018/346). Depends on 360 (the tagged `Finding` +
decay data).
...
(§3c). Next (after 360 GREEN): the six walkable disciplines + `develop.review`/
