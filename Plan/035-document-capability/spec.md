...
- [ ] Supported scopes (v1):
  - `"install-artefacts"` — all `Reflection` nodes with
    `kind=install-artefact` (Spec 017 §"Why").
  - `"reflections"` — recent Reflection nodes, optionally filtered by
    `intent_id`.
  - `"provenance"` — same shape as `memory_graph_provenance(intent_id)`
    but as rendered markdown (a per-intent provenance brief).
  - `"capability-catalogue"` — the engine's capability map (the doc
    surface today regenerated by `agency.install` into
