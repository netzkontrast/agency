...
# Spec 360 — Migration: brooks-lint plugin → native agency quality surface

> Part of the Spec 353 brooks-lint port. This slice is the **transition path** for
...
Spec 353 "replaces brooks-lint itself" — but a project that *already runs*
brooks-lint has state on disk: a `.brooks-lint.yaml` config and a
`.brooks-lint-history.json` trend. The port specs (356) say these "disappear" —
...
- Maps each key onto the unified `quality:` block (356 §2), preserving semantics
...
- **Compat window:** 356 already reads `.brooks-lint.yaml` for one release
  (356 OQ1), agency config winning on conflict. This verb makes the migration
  *explicit + durable*; the auto-read is the *safety net* during the window. After
  the window, the auto-read is removed (a 356 follow-up) and the file is inert.
...
- Mints one `QualityRun{mode, scope, score, critical, warning, suggestion,
  recorded_at, status: complete}` per record (356 §3), back-dating `recorded_at`
...
1. **Native lands** (354–359 shipped) — `develop.review` live, both surfaces
   coexist (the native uses `quality-*` names; brooks uses `/brooks-*` — Spec 355
...
4. The brooks-lint **repo stays** as the cited upstream the data is vendored from
   (Spec 353 — like ponytail for `frugal`); deinstalling the *plugin* ≠ deleting
   the *reference*.

### 4. Vendored-data versioning (cross-ref — owned by 354/358)

The `_source: brooks-lint@<rev>` provenance keys on `decay-risks.json` (354) and
`source-coverage.json` (358), checked by `check-drift`, are the *forward*
...
- No new config schema (356 owns `quality:`) — it *writes* it from the legacy file.
- No new `QualityRun`/`Suppression` shape (356) — it *imports into* them.
- No destructive deletion of the legacy files (keep-both; the user removes them).
- No permanent dual-read shim — the 356 auto-read is a bounded compat window; this
  verb is the durable migration that lets the window close.
- No re-vendoring of the decay-risk data (354/358 own the data + its `_source`).
...
Scenario: suppress entries become Suppression nodes via intent.triage
  ...
  And the next review downgrades that finding (356 §4)
...
**DRAFTED 2026-06-20** — the migration/transition slice of the Spec 353 program,
opened by the spec-panel critique (Newman — the completeness gap the critical-
thinking pass and the panel both flagged). No code yet. Reuses 356 (`quality:`
config + `QualityRun`/`Suppression`), `intent.triage` (356 §4), keep-both (292).
...
the vendored-data `_source` read. Lands alongside 358/359 (late — needs 356's
config + nodes live). Next (after 356): the two importers + the first-review
