...
# Spec 371 — Brooks-lint port (judgment-based code-quality lens, native + develop-integrated)
...
| 6 modes: review · audit · debt · test · health · sweep | ❌ | **Spec 372** — six walkable skills + `develop.review` seam |
| `--fix` remedy mode (auto-apply safe, confirm risky) | ❌ | **Spec 372** — remedy phase behind a gate |
| Health Score (base 100; strict/balanced/legacy-friendly presets) | ❌ | **Spec 373** — computed score + presets |
| `.brooks-lint.yaml` config (disable/severity/ignore/focus/strictness/suppress/custom_risks) | ⚠️ unified `config` (Spec 334) exists | **Spec 373** — map onto agency config |
| history `.brooks-lint-history.json` + trend | ❌ (graph + `manage.timeline`) | **Spec 373** — runs as graph nodes; trend is a query |
| triage (accept/dismiss/defer/skip) + suppress w/ expiry | ❌ | **Spec 373** (scoring read) + **`intent.triage`** verb (owner directive — triage is an intent judgment) |
| 12 markdown prose files (Report Template · 6 mode guides · remedy · source-coverage · decay-risks · custom-risks) | ⚠️ template surface exists (Spec 060) | **Spec 376** — ported as agency templates (`<!-- AGENT: -->`) + `develop.reference` docs |
| SARIF output (`sarif.mjs`) | ❌ | **Spec 374** — `analyze.sarif` emit |
| report-parse (`report-parse.mjs`) | n/a — findings already structured | **Spec 374** — *dropped* (port-forward: no prose to parse) |
| CI GitHub Action + ci-gate | ⚠️ `.github/workflows/test.yml` + `gate` cap | **Spec 374** — review-on-diff CI step + score gate |
| 12-book source-coverage discipline | ❌ | **Spec 375** — source-coverage data + cite-discipline |
| eval suite (57 scenarios) + parser benchmark (frozen 30 reports) | ❌ (Gherkin acceptance, Spec 053/077) | **Spec 375** — translate to Gherkin + SARIF property tests |
...
findings (Spec 372). Both record as `Finding` graph nodes (Goal 2).
...
- **The engine extends `analyze/`** — the `Finding` shape (354), the decidable
  risk-code scanners (354), score/config/triage/history (373), SARIF (374). This
...
- **The developer-facing entry is `develop.review`** — "integrate linting INTO
  ...
  modes are walkable skills it drives (372). `develop.review(mode, scope)`
  ...
  **templates** + `develop.reference` docs (Spec 376).
- **One judgment touchpoint in `intent`** — `intent.triage` (373 §4, owner
...
Each mode skill (372) walks the same shape, one phase at a time (bounded context):
...
3. **Judgment pass** — the agent reads the decay-risk data (354) for the
   ...
   (the source-coverage cite-discipline, 375).
4. **Score + report** — Health Score computed from findings × preset (373);
   render the Iron Law report (374).
5. **Remedy/triage** (optional, gated) — `--fix` applies safe remedies, confirms
   risky ones; triage records suppressions (373).
...
371 (this umbrella)
 └─ 354  decay-risk Finding shape + knowledge data   ← FOUNDATION (no dep)
     ├─ 372  review/remediate + 6 modes + headless twin ← needs 354
     ├─ 373  Health Score + config + history          ← needs 354
     │        + intent.triage (suppression write)     ← + intent (091)
     ├─ 374  SARIF + CI + quality gate (+ ops hardening) ← needs 354 (+373 score)
     ├─ 375  acceptance + source-coverage + evals      ← needs 354,372
     ├─ 376  brooks PROSE → templates + references      ← needs 354,372,374
     └─ 377  migration (.brooks-lint.* → config+graph)  ← needs 373

POST-v1 followup (NOT in the build order — land on demand, YAGNI):
 ⋯ 378  multi-language decidable scanners              ← needs 354,372
```

354 ships first (everything depends on the extended `Finding` + the data). 372
and 373 are parallelizable on top of 354. 374 needs the score from 373. 375
(tests/docs) + 376 (prose/templates) + 377 (migration) land last: 376 renders
374's report path and binds 372's mode skills; 377 imports into 373's config +
nodes; 375 validates the rest. The triage verb (`intent.triage`) is an `intent`
addition carried under 373 (a `Finding` SERVES an intent, so triaging it is a
...
> the operational axis (374) — the panel's weakest at 5.5/10.
...
- **OQ1 — config file name.** Keep `.brooks-lint.yaml` for drop-in compat, or
  fold entirely into `.agency/config.yaml` under a `quality:` block? (373 default:
...
- **OQ3 — `develop.review` vs `analyze.review`. RESOLVED (spec-panel, 2026-06-20):**
  ...
  the CI entry). Not one verb; two front doors, one engine (372 §3a).
...
analyze+develop-seam · engine=hybrid · 5+1 specs). Children 354 + 372–375 drafted in
the same commit.
...
dedicated child **Spec 376** (the "use EVERYTHING from brooks" prose layer the
...
`Suppression`/`Acknowledgement` nodes on the `intent` ontology; carried as a 373
amendment.
...
(effect), since `@verb(role=)` is static (372); Wiegers — the Iron Law gate is a
**decidable predicate** over `Finding` fields, not agent self-assertion (354/372).
**(architecture)** Cockburn+Hightower — the headless/CI actor is the first-class
`analyze.review` twin over a shared `_review.py` core (372 §3a, resolves OQ3);
Hohpe — the decidable→judgment **merge contract** (one `Finding` per
`(risk_code, span)`, 354/372). **(operational, the 5.5/10 axis)** Hightower —
keyless decidable-only CI degradation + base-branch `.agency` cache so trend
survives ephemeral CI (373/374); Nygard — SARIF size cap with a locator, a
wedged-PR override, partial-walk `status:incomplete` never reports green (373/374).
**(testing)** Crispin — stated language matrix (decidable=py, judgment=any) +
deterministic/`-m wet` scenario split + per-mode coverage (372/375); Adzic —
fixture-backed scenarios, not prose Givens (375). **(completeness)** Newman — the
**migration** path is now dedicated child **Spec 377** (`.brooks-lint.*` →
config+graph) + symmetric vendored-data `_source` versioning (354/375).

The program is now **umbrella + 7 children (354 + 372–377)**, with **Spec 378**
...
"good call", 2026-06-20), and the multi-language lift is captured as 378 rather
than expanding 354/372 now (YAGNI). Next: 354 (foundation) TDD.
