...
owner: "@agency"
vision_goals: [4, 2, 6]
depends_on: ["371", "354", "372", "374", "060", "292"]
domain: analyze
wave: brooks-port
parent_spec: "371"
---

# Spec 376 — Brooks prose → agency templates + references (the develop-lint output surface)

> Part of the Spec 371 brooks-lint port. This slice carries the **prose** —
...
decay and HOW to shape the report. Specs 354–375 port the structured half
(findings, score, SARIF, config, evals); without the prose they are a skeleton
with no diagnostic voice. "Use everything from the Brooks-Lint system" (Spec 371)
...
|---|---|---|
| shapes **output** (the report a human reads) | a render **template** (`<cap>/templates/`) | `ctx.render` → `document.render` (374) |
| shapes **judgment during the walk** (how to diagnose, what NOT to flag) | an on-demand **reference** | `develop.reference("<topic>")` |
| is **fact/registry** (risk defs, book matrix, score weights) | **data** (`<cap>/data/*.json`) | 354 / 373 / 375 |
...
template, rendered graph→file by `document.render` (374 §4). It carries the
...
<!-- AGENT: render the Config: line here ONLY when a config was applied
     (preset, N disabled, M ignored) — Spec 373 §2. Omit otherwise. -->
...
<!-- AGENT: collapsed "Suppressed" section when suppressions matched (373 §4);
     not counted in the score. -->
$suppressed_block

## Summary
<!-- AGENT: 2–3 sentences. Under legacy-friendly, LEAD with the three
     highest-leverage fixes (373 §1). -->
$summary
...
<!-- AGENT: $fix_tier_label is the [quick-fix]/[guided]/[manual] tag ONLY under
     ...
     Consequence or Remedy is REJECTED upstream (the Iron Law gate, 372 §2) —
     this template never renders a partial finding. -->
```

This makes the Iron Law a **rendered invariant**: the template has four required
slots; the judgment gate (372) guarantees they are filled before render.
...
The mode skill (372 §2) renders its template at the matching phase, so the engine
...
- `develop/templates/quality-remedy.md` ← `remedy-guide.md`: the
  ...
  in the remedy phase (372 §4, `--fix`). Carries brooks' "What NOT to do"
...
|---|---|---|
| `"decay-risks"` | `decay-risks.md` + `test-decay-risks.md` | before the judgment pass (372 §2 phase 3) |
| `"source-coverage"` | `source-coverage.md` | before writing findings (the cite-discipline, 375) |
| `"remedy"` | `remedy-guide.md` (the heavy how-to) | before `--fix` (paired with §4's template) |
| `"custom-risks"` | `custom-risks-guide.md` | when config declares `custom_risks` (373 §2) |
...
> **facts** → JSON data (354/373/375). The reference prose POINTS AT the data
...
- **No finding/score/SARIF logic** (354/373/374) — it renders THEIR output and
...
- **No prose-parsing** (`report-parse.mjs` stays dropped, 374) — templates render
...
**DRAFTED 2026-06-20** — the prose/output slice of the Spec 371 program, opened by
...
templates (render output), references (guide judgment), and data (354/373/375).
Depends on 354 (Iron Law fields the template renders), 372 (mode skills that bind
the walk templates), 374 (`document.render` path). Lands alongside 374/375. Next
(after 354/372): author `quality-report.md` + `iron-law-finding.md` +
