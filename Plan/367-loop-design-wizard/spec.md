...
> Child of Spec 362. Ports looper's **`SKILL.md` 7-stage interview** and the
...
where each phase `produces` keys, optionally `gate`s, and surfaces guidance. 367
defines the `loop-design` skill whose seven phases each delegate to a sibling verb
from 363–366/368 and load that stage's rubric.
...
SKILL = {
  ...
  "phases": [
    phase(1, "goal",        produces=["goal_id"],        verbs=["loop.frame_goal","loop.critique_goal"],
          # loads data/rubrics/goal-rubric.md; gate: clarity advisory (363)
         ),
    phase(2, "verification", produces=["criteria"],       verbs=["loop.add_criterion","loop.verify_report"],
          gate="has_non_vibe_criterion"   # 364 — warn if all-vibe, hard-gate only on explicit accept
         ),
    phase(3, "host",         produces=["host_member"],    verbs=["loop.add_member","loop.detect_models"],
          # which model drives the loop (365/369)
         ),
    ...
    phase(7, "emit",         produces=["emitted"],        verbs=["loop.compile","loop.emit","loop.render_handoff"],
          # 368 — write loop.yaml/resolved/LOOP.md/RUN_IN_SESSION.md; offer in-session run (366)
         ),
  ],
}
```
...
- **Advisory phases** (goal, verification) surface findings (363 `critique_goal`,
  364 `verify_report`) but the walker proceeds — matching looper's "warn, don't
...
- **Hard-gated phases** (council, control) **block advancement**: phase 4 will not
  ...
  `revise_until_clean` gate (the reviewer-only rule, 365); phase 5 will not pass
  until `open_loop` accepts a termination-guarded control (366). These are the two
...
looper shows before emission — derived from the machine (366) + gates (364) +
council (365), optimized for Claude Code CLI width:
...
Scenario: emit produces the artefacts and offers an in-session run
  ...
  Then loop.yaml, loop.resolved.json, LOOP.md, and RUN_IN_SESSION.md exist (368)
  And the wizard offers to advance the loop machine in-session (366)
...
- [ ] Phase 7 drives `compile`/`emit`/`render_handoff` (368) and offers an in-session machine walk (366).
...
**Verdict:** Not started — drafted under the 362 wave. Looper's 7-stage interview
as a `loop-design` walkable skill (`SkillRun`, 346) composing 363–366/368; rubric
...
Depends on all four foundation children + emission (368); the user-facing entry
to the cluster.
