...
owner: "@agency"
vision_goals: [6, 2]
depends_on: ["371", "354", "372", "053", "077"]
domain: analyze
wave: brooks-port
parent_spec: "371"
---

# Spec 375 — Acceptance coverage + source-coverage + eval/benchmark translation
...
The **cite-discipline** is a phase the judgment pass (372) reads on demand
...
SARIF validity. Agency has no parser (findings born structured, Spec 374), so this
...
- Properties asserted (computed, not pinned): SARIF is valid 2.1.0; the rule set
  equals the live risk-code registry (Spec 374 §1 — drift-proof); every finding
...
- No new findings/score/SARIF (354/373/374 own those) — it *tests* and *grounds*
  them.
...
**DRAFTED 2026-06-20** — the grounding/verification slice of the Spec 371 program.
...
drift). Depends on 354 (risk registry) + 372 (modes to exercise). Lands last,
validating 354–374.
...
354/372): `source-coverage.json` (+ `_source` marker) + the paired fixture-backed
